...
**UI 方案**（待确认）：在消息底部 model label 旁添加 "..." 按钮，点击弹出 `DropdownMenu`，包含 "Fork from here" 选项。Android 当前 model label 仅显示在 assistant 消息上，与 iOS（显示在 user 消息上）不一致，需确认统一策略。可选方案：A）跟随现有 model label 位置（assistant 消息）；B）统一到 user 消息（与 iOS 一致）；C）两边都放。
